...
Further, since Oracle workloads can typically be quite demanding from a storage performance (IOPS) point of view, often times - due to IOPS limitations - larger VM SKUs need to be selected to handle the workload. This then might result into an unnecessary increase in core count - which impacts lisencing. When using [Azure NetApp Files](https://docs.microsoft.com/en-us/azure/virtual-machines/workloads/oracle/oracle-vm-solutions#shared-storage-configuration-options) as the shared storage option, the IOPS do not count towards the VM IOPS limit, but rather to the VM network bandwidth limitations. This means that typically smaller VM SKUs (as mentioned above) can be used to handle the Oracle workload, lowering Oracle licensing cost. It is recommended to choose a VM SKU with [Accelerated Networking](https://azure.microsoft.com/en-us/blog/maximize-your-vm-s-performance-with-accelerated-networking-now-generally-available-for-both-windows-and-linux/) enabled. Oracle benchmark information on Azure NetApp Files can be found [here](https://aka.ms/ANF-benchmarks)
...
Leveraging Azure NetApp Files, with a standard [99.99% availability SLA](https://azure.microsoft.com/en-us/support/legal/sla/netapp/v1_1/), will further help in designing high available architectures when used in combination with availability sets or availability zones.
